  ...
  state = {
    ...
    addPrize = (e, id) => {
        ...
        if(this.state.prizePoints > 0 && this.state.prizePoints < 1001 && this.state.prizePoints > this.state.leaderPoints){
            axios.post(`http://95.108.36.173:8081/api/habits/habit/setPointsToWIn?habitID=${id}&pointsToWin=${this.state.prizePoints}`)
            .then(res => {
    ...
    componentDidMount() {
        ...
        axios.get(`http://95.108.36.173:8081/api/members/leader/points?habitID=${this.props.habitID}`)
        .then(res => {
            this.setState({
