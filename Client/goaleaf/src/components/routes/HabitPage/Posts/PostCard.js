...
class PostCard extends Component {
    ...
    addReaction = reaction => {
        axios.post('http://95.108.36.173:8081/api/posts/post/addreaction', {
    ...
    handleCommentCardDeleted = (id) => {
        axios.delete(`http://95.108.36.173:8081/api/comments/remove?id=${id}`)
            .then(res => {
    ...
    addComment = e => {
        e.preventDefault();
        this.clearMsg();
        axios.post('http://95.108.36.173:8081/api/comments/addcomment', {
    ...
    showCommentsBtnPress = () => {
        ...
        axios.get(`http://95.108.36.173:8081/api/comments/post/paging?pageNr=${0}&objectsNr=${this.state.commentsToShow}&postID=${this.props.id}`)
        .then(res => {
            this.setState({
    ...
    showComments = (page, objectsNr, postID) => {
        ...
        this.setState({commentsLoading: true});
        axios.get(`http://95.108.36.173:8081/api/comments/post/paging?pageNr=${page}&objectsNr=${objectsNr}&postID=${postID}`)
            .then(res => {
    ...
    handleTaskDeleted = (id) => {
        axios.delete(`http://95.108.36.173:8081/api/tasks/task/pushback?postID=${id}`)
            .then(res => {
    ...
    componentDidMount() {
        axios.get(`http://95.108.36.173:8081/api/posts/presentreaction?postID=${this.props.id}&userLogin=${this.props.currentUserLogin}`)
            .then(res => {
